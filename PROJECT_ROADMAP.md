...
- [x] **1.1 Home Page Flow Adjustment (`src/pages/Home.tsx`)**
  - Change all event card buttons from direct "Book Ticket" to "View Details" / "Select Ticket" pointing to `/event/:id`.
- [x] **1.2 Backend Delete Event Protection (`backend/index.js`)**
...
- [x] **1.3 Venue & Schedule Fields Expansion (`AddEvent.tsx` & `EditEvent.tsx`)**
  - Add inputs for detailed venue address, Google Maps link, open gate time, show start time, and terms & conditions.
- [x] **1.4 Admin Dashboard Manual/Auto Refresh (`src/pages/Admin.tsx`)**
  - Add a "Refresh Data" button and background polling to update transaction tables live.
- [ ] **1.5 JWT Authentication & Middleware Security (`jsonwebtoken`)** *(Pending until deployment)*
...
- [x] **2.1 User Profile Management (`src/pages/Profile.tsx`)**
